...
function SetCollegeProfile() {
  ...
  const handleClick = (e) => {
    ...
    console.log(fd);
    axios
      .patch("http://localhost:5000/college/update/profile", fd, data)
      .then((resp) => {
        if (resp.status === 202) {
  ...
  return (
    <Container fluid>
      ...
      <div
        style={{ fontSize: "180%", textAlign: "center", marginBottom: "3%" }}
      >
      ...
      <Form>
        ...
        <div style={{ display: "flex", justifyContent: "center", }}>
          <Button onClick={handleClick} className="but1 px-4 py-1" style={{background: "linear-gradient(10deg, #D63484, #1d4ed8)", }} >
            SUBMIT
          </Button>
        </div>
      </Form>
